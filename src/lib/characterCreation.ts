...
export function getAttributePointsSpent(
  character: Pick<Character, "stats">,
): number {
  return Object.values(character.stats).reduce((sum, value) => sum + value, 0);
}
export function getAttributePointsRemaining(
  character: Pick<Character, "stats">,
): number {
  return (
    CHARACTER_CREATION_RULES.attributePoints -
    getAttributePointsSpent(character)
  );
}
export function getRequiredSkillPoints(): number {
  return Object.values(CHARACTER_CREATION_RULES.requiredSkills).reduce(
    (sum, value) => sum + value,
    0,
  );
}
...
export function getCreationPointSummary(
  character: Pick<Character, "stats" | "skills">,
  ...
  return {
    ...
    attributePointsRemaining:
      CHARACTER_CREATION_RULES.attributePoints - attributePointsSpent,
    ...
    skillPointsRemaining:
      CHARACTER_CREATION_RULES.skillPoints - skillPointsSpent,
  };
}
export function canIncreaseAttribute(
  ...
  return (
    character.stats[attribute] < ATTRIBUTE_CREATION_RULES[attribute].maximum &&
    getAttributePointsRemaining(character) > 0
  );
}
export function canDecreaseAttribute(
  ...
  return (
    character.stats[attribute] > ATTRIBUTE_CREATION_RULES[attribute].minimum
  );
}
export function canIncreaseSkill(
  ...
  return Boolean(
    skill &&
    skill.level < CHARACTER_CREATION_RULES.skillMaximum &&
    getSkillPointsRemaining(character) > 0,
  );
}
...
export function changeAttributeAtCreation(
  character: Character,
  attribute: AttributeName,
  delta: 1 | -1,
): Character {
  if (
    (delta === 1 && !canIncreaseAttribute(character, attribute)) ||
    (delta === -1 && !canDecreaseAttribute(character, attribute))
  )
    return character;
  const stats = {
    ...character.stats,
    [attribute]: character.stats[attribute] + delta,
  };
  const maximumHitPoints = calculateMaximumHitPoints(stats);
  return {
    ...character,
    stats,
    skills: recalculateSkills(stats, character.skills),
    combat: {
      ...character.combat,
      hp: { current: maximumHitPoints, max: maximumHitPoints },
    },
  };
}
export function changeSkillAtCreation(
  character: Character,
  skillId: string,
  delta: 1 | -1,
): Character {
  if (
    (delta === 1 && !canIncreaseSkill(character, skillId)) ||
    (delta === -1 && !canDecreaseSkill(character, skillId))
  )
    return character;
  const skill = character.skills[skillId];
  const skills = {
    ...
    [skillId]: {
      ...skill,
      level: skill.level + delta,
      base: character.stats[skill.stat] + skill.level + delta,
    },
  };
...
export function validateCharacterCreation(
  ...
  if (summary.attributePointsRemaining < 0)
    errors.push("Os pontos de atributos foram excedidos.");
  if (summary.skillPointsRemaining < 0)
    errors.push("Os pontos de perícias foram excedidos.");
  for (const attribute of Object.keys(character.stats) as AttributeName[]) {
    ...
    if (value < rule.minimum || value > rule.maximum)
      errors.push(
        `${attribute} deve ficar entre ${rule.minimum} e ${rule.maximum}.`,
      );
  }
  for (const [id, skill] of Object.entries(character.skills)) {
    if (skill.level < getSkillMinimum(id))
      errors.push(
        `${skill.name} não pode ficar abaixo de ${getSkillMinimum(id)}.`,
      );
    if (skill.level > CHARACTER_CREATION_RULES.skillMaximum)
      errors.push(`${skill.name} ultrapassou o limite de criação.`);
  }
...
export function finalizeCharacterCreation(character: Character): Character {
  ...
  return {
    ...character,
    skills: recalculateSkills(character.stats, character.skills),
    progression: { improvementPoints: 0 },
  };
}
