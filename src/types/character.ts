...
export interface Character { id: string; schemaVersion: 2; createdAt: string; updatedAt: string; identity: CharacterIdentity; stats: Stats; skills: Skills; primaryRole: RoleId | null; roleAbilities: RoleAbilityState[]; ip: number; /** Special Moves desbloqueados (1 ponto da forma por move); undefined em fichas antigas = nenhum. */ unlockedSpecialMoves?: string[]; teamMembers: TeamMember[]; familyVehicles: FamilyVehicle[]; progression: Progression; wallet: Wallet; rollHistory: RollHistoryEntry[]; lastAttack?: import("@/types/attack").AttackRollResult; lastDamage?: import("@/types/attack").DamageRollResult; combat: CombatStats; humanity: { current: number; max: number }; luck: { current: number; max: number }; weapons: Weapon[]; cyberware: CyberwareItem[]; inventory: InventoryItem[]; }
const defaultStats: Stats = { INT: 2, REF: 2, DEX: 2, TECH: 2, COOL: 2, WILL: 2, LUCK: 2, MOVE: 2, BODY: 2, EMP: 2 };
export function createEmptyCharacter(id = crypto.randomUUID()): Character { const now = new Date().toISOString(); const stats = { ...defaultStats }; return { id, schemaVersion: 2, createdAt: now, updatedAt: now, identity: { name: "", player: "", role: "", level: 1 }, stats, skills: createDefaultSkills(), primaryRole: null, roleAbilities: [], ip: 0, unlockedSpecialMoves: [], teamMembers: [], familyVehicles: [], progression: { improvementPoints: 0 }, wallet: { eurodollars: 0 }, rollHistory: [], combat: { hp: { current: calculateMaximumHitPoints(stats), max: calculateMaximumHitPoints(stats) }, stamina: { current: 0, max: 0 }, armor: { head: 0, body: 0 }, criticalInjuries: [], deathSaveDC: 0, deathSaveFailures: 0, isDead: false }, humanity: { current: calculateMaximumHumanity(stats), max: calculateMaximumHumanity(stats) }, luck: { current: 0, max: 0 }, weapons: [], cyberware: [], inventory: [] }; }
